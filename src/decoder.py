...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, enc_output: torch.Tensor, tgt_mask: torch.Tensor) -> torch.Tensor:
        ...
        attn_output = self.self_attention(hidden_state, hidden_state, hidden_state, tgt_mask)
        x = x + attn_output

        # Apply layer normalization and cross-attention
        hidden_state = self.layer_norm_2(x)
        cross_attn_output = self.cross_attention(hidden_state, enc_output, enc_output)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, enc_output: torch.Tensor) -> torch.Tensor:
        ...
        tgt_mask = torch.triu(torch.full((seq_len, seq_len), float('-inf'), device=x.device), diagonal=1)
